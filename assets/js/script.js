...
const sizeInput = document.getElementById('sizeInput'); // input 
const sortButton = document.getElementById('sortButton'); // button tri
const resetButton = document.getElementById('resetButton'); // button Réinitialiser
const itemList = document.getElementById('itemList'); // list
const itemLists = document.querySelectorAll('#itemList li');
const countDisplay = document.getElementById('count');  // span 0 1 2...
...
function filterItemsBySize(maxSize) {
  ...
  for(let i = 0;i<items.length;i++){
    ...
    if(items[i].textContent.length <= maxSize){
